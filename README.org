...
* Features
- Hybrid SPI path: polling for command bytes, DMA for 512-byte sector payloads
- Semihosting logs are emitted for driver error paths
...
* How it Works
The library uses blocking SPI transfers for command, status, CRC, and clock bytes so short
transactions complete before the stack frame changes. 512-byte sector payloads use DMA, and
the =dma_complete= flag is paired with a timeout guard so stalled DMA transfers are aborted
cleanly instead of hanging the driver. During card init the driver forces a safe low SPI
prescaler, then switches to a fast transfer prescaler after initialization completes. The
library emits semihosting messages for DMA timeouts, bad data tokens, command timeouts,
and init failures in every build unless =SD_SEMIHOST_LOG=0= is defined at compile time.
